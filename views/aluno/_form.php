...
doc.text("- Usar as piscinas com roupa apropriada (sunga, maiô - se possível do uniforme)", 22, 81);
doc.text("- Não é permitido o uso de cigarros e chicletes, bem como de bronzeadores ou creme.", 22, 86);
doc.text("- Tomar uma ducha (fria) antes de entrar na piscina, retirando os resíduos de suor.", 22, 91);
doc.text("- É proibido fazer atividades nos salões só de maiô ou sunga.", 22, 96);
doc.setFont("times", "bold");
doc.text("HORÁRIO:", 22, 101);
doc.setFont("times", "normal");
doc.text("- Faça um bom uso das dependências da Harmonia, nos horários combinados na secretaria.", 22, 107);
doc.text("- Aguarde nas salas de espera o horário de suas atividades.", 22, 113);
doc.text("- Permaneça nos vestiários só o tempo necessário para a higiene e troca de roupa.", 22, 118);
doc.text("- Não fazemos reposição de aulas perdidas.", 22, 123);
doc.setFont("times", "bold");
doc.text("SEGURANÇA:", 22, 128);
doc.setFont("times", "normal");
doc.text("- Acompanhantes só poderão ultrapassar a sala de espera quando solicitados.", 22, 134);
doc.text("- Quando houver necessidade de conversar com o professor ou com o fisioterapeuta comunique-se", 22, 140);
doc.text("com a recepção, ou procure a coordenação.", 22, 145);
doc.text("- Ficam disponíveis armários nos banheiros. Traga seu cadeado para trancar seus pertences enquanto ", 22, 150);
doc.text("faz suas atividades.", 22, 155)
doc.text("- Crianças, bebês e pessoas com necessidades especiais devem ser trocados no local apropriado.", 22, 160);
doc.text("- PEGUE SUA FICHA DE BANHO NA SECRETARIA.", 22, 165);
doc.setFont("times", "bold");
doc.text("PAGAMENTO:", 22, 170);
doc.setFont("times", "normal");
doc.text("- Os pagamentos serão efetuados na secretaria de 07:30 as 20:00hs (no sábado até as 11:30hs).", 22, 176);
doc.text("- Há desconto de 5% para pessoas da mesma família.", 22, 182);
doc.text("- Para fazer jus aos descontos combinados e evitar multas, é necessário absoluta pontualidade.", 22, 187);
doc.text("- O mês é sempre pago adiantado, vencendo no dia 1º, podendo ser quitado até dia o 5º útil.", 22, 192);
doc.text("- O aluno só terá direito ao desconto na mensalidade, somente com comprovação médica", 22, 197); 
doc.text("(Não está incluído faltas por férias, viagens...).", 22, 202);
doc.text("- Ao parar de fazer aula, o aluno poderá retornar até 60 dias sem cobrança da taxa de matrícula.", 22, 207);
doc.text("(Só garantimos o mesmo horário com o pagamento mensal integral).", 22, 212);

doc.setFont("times", "bold");
doc.text("AUTORIZAÇÃO DE USO DE IMAGEM:", 22, 222);
doc.setFont("times", "normal");
doc.text("[ ] Desde já, autoriza a Harmonia, com anuência de seu representante legal, caso for,", 22, 228);
doc.text("a utilizar sua imagem, quando dentro de suas dependências ou em aulas e eventos externos,", 22, 233);
doc.text("para fins de divulgação nos diversos meios de comunicação, sem qualquer tipo de contraprestação.", 22, 238);
